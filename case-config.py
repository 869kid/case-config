...
# Если измените эти SCOPES, удалите файл token.pickle.
SCOPES = ['https://www.googleapis.com/auth/spreadsheets.readonly',
          'https://www.googleapis.com/auth/drive.file']

def get_service():
    creds = None
    # Файл token.pickle сохраняет токены доступа и обновления, и создается автоматически при первом выполнении авторизационного потока.
...
def parse_cases(cases_values, rewards_table):
    ...
    while group_idx < len(cases_values[1]):
        if "Group" in cases_values[1][group_idx]:
            ...
            if rewards:
                case_config[case_name]["groups"][group_name] = {
